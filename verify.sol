...
// File contracts/TokenineRewardPoint.sol
...
contract MVPDonate is ERC20("Stake MDD", "ST-MDD") {
  ...
  function calculateRewardPoint14days(uint256 _amount)
    public
    view
    returns (uint256)
  {
  ...
  function leave(uint256 _share) public {
    require(
      endDate <= currentTime(),
      "ERC20: Unlock on Friday, December 31, 2021 0:01:00 GMT+07:00"
    );
    uint256 totalMVP = mvp.balanceOf(address(this));
    uint256 totalShares = totalSupply();
...
// File contracts/mvpstakebtc.sol
...
contract MVPStakeBTC is ERC20("Stake MVPBTC", "ST-MVPB") {
  ...
  function leave(uint256 _share) public {
    require(
      ...
      "ERC20: Unlock on July 10, 2021 5:30:32 PM GMT+07:00"
    );
    uint256 totalMVP = mvp.balanceOf(address(this));
    uint256 totalShares = totalSupply();
    uint256 what = _share.mul(totalMVP).div(totalShares);
